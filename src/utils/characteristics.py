...
def calc_price_with_roll(i: int, price_type: str):
    ...
    return ([
        ...
        pl.when(pl.col('exp_1') == 1)
        .then(pl.col(f'{price_type}_local_{i + 1}'))
        .otherwise(pl.col(f'{price_type}_local_{i}'))
        .over(pl.col('clscode'))
...
def calc_returns_until_expiry(i: int, price_type: str):
    ...
    return ([
        ...
        pl.when((pl.col('daystomaturity_1').shift(1) == 0) | (pl.col('lasttrddate_1') == pl.col('lasttrddate_2').shift(1)))
        .then((pl.col(f'{price_type}_local_{i}') / pl.col(f'{price_type}_local_{i + 1}').shift(1)) - 1)
        .otherwise(((pl.col(f'{price_type}_local_{i}') / pl.col(f'{price_type}_local_{i}').shift(1)) - 1))
...
def calc_returns_with_price_adj_and_roll(i: int):
    ...
    return ([
        ...
        pl.when(pl.col('exp_1') == 1)
        .then(pl.col(f'ret_temp_local_{i + 1}'))
        .otherwise(pl.col(f'ret_temp_local_{i}'))
        .over(pl.col('clscode'))
        .alias(f'ret_local_{i}'),
    ])
...
def calc_total_returns_with_roll(i: int, price_type: str):
    ...
    return ([
        ...
        pl.when(((pl.col('exp_1') == 1) & (pl.col('switch_1') == 1)) & (pl.col(f'daystomaturity_1').shift(1) != 0))
        .then((pl.col(f'{price_type}_local_{i + 1}').shift(1) / pl.col(f'{price_type}_local_{i}').shift(1)) * (pl.col(f'ret_temp_local_{i + 1}') + 1) - 1)
        .otherwise(pl.col(f'ret_local_{i}'))
        .over(pl.col('clscode'))
