...
def load_quotes_data(path: str) -> pl.DataFrame:
    ...
    """
    return (
        pl.scan_csv(TICKHISTORY_PATH / "quotes" / path, schema_overrides={"Close Bid": pl.Float64, "Close Ask": pl.Float64, "GMT Offset": pl.Int8})
...
def count_expiries_between(row_date: date, row_trad_last: date, expiry_list: list[date]) -> int:
...
def process_future(FUTURE: Future) -> pl.DataFrame:
    ...
    # Before calculating returns, convert prices to USD using FX rates
    if FUTURE.curcdd != "USD":
        tousd = (
            TOUSD_RATES
            .select(pl.col("date"), pl.col(FUTURE.curcdd).cast(pl.Float64).alias("tousd"))
            .sort("date")
        )
        daily_vwaps = (
            daily_vwaps
            .join(tousd, left_on="date_", right_on="date", how="left")
            .with_columns([
                (pl.col("front_month_settlement") * pl.col("tousd")).alias("front_month_settlement"),
                (pl.col("settlement_c1") * pl.col("tousd")).alias("settlement_c1"),
                (pl.col("settlement_c2") * pl.col("tousd")).alias("settlement_c2"),
                (pl.col("settlement_c3") * pl.col("tousd")).alias("settlement_c3"),
                (pl.col("settlement_c4") * pl.col("tousd")).alias("settlement_c4"),
            ])
            .sort("date_")
        )
...
def main():
    ...
    for FUTURE in tqdm(CONFIG):
        ...
        returns_data.append(process_future(FUTURE))
    ...
    returns_wide.write_csv(DATASETS_ROOT / "tier1" / "sync" / f"{ASSET_CLASS.name.lower()}_daily_returns.csv")
...
if __name__ == "__main__":
    ...
    CONFIG = []
    for future in load_config(PROJECT_ROOT / "tier1.yaml"):
    ...
    TOUSD_RATES = pl.read_csv(COMPUSTAT_PATH / "merged_tousd.csv", schema_overrides={"date": pl.Date}).fill_null(strategy="forward")
