...
from globalmacro.validation.base import grade, write_summary
...
def _available_checks():
    checks = [datastream_check, consistency_check]
    try:
...
def main(argv=None):
    ...
    for check in _available_checks():
        ...
        correlations.write_csv(str(out_dir / "correlations.csv"))
        r = grade(check.name, check.slug, correlations)
        results.append(r)
    ...
    write_summary(results, VALIDATION_OUTPUT / "VALIDATION_SUMMARY.md")
