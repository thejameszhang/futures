...
from scripts.detect_sync_cross_contract import cross_contract_cells, run, summarize
...
# ---------------------------------------------------------------------------
# scripts/detect_sync_cross_contract.py: the independent verification detector.
# It must never import provenance_break_mask, apply_provenance_guard,
# attach_front_slot, rescue_ret1_adjusted, or is_rescuable_mask, and must never
# reconstruct them -- see its module docstring. The functions imported at the top
# of this file (attach_front_slot, apply_provenance_guard) are used below ONLY to
# build ground truth for fixtures, exactly as the task that specified this
# detector allows; the detector itself never sees them.
# ---------------------------------------------------------------------------


def test_detects_a_cross_slot_return():
    df = pl.DataFrame({"date_": [1, 2], "settlement_c1": [100.0, 110.0],
                       "settlement_c2": [200.0, 220.0],
                       "ret1_adjusted": [None, 110.0 / 200.0 - 1]})
    assert cross_contract_cells(df)["date_"].to_list() == [2]


def test_accepts_a_same_slot_return():
    df = pl.DataFrame({"date_": [1, 2], "settlement_c1": [100.0, 110.0],
                       "settlement_c2": [200.0, 220.0],
                       "ret1_adjusted": [None, 110.0 / 100.0 - 1]})
    assert cross_contract_cells(df).height == 0


def test_a_value_no_slot_pair_reproduces_is_undecidable_not_flagged():
    # 0.5 matches none of the four (num, den) candidates buildable from c1/c2 --
    # silence here is honest uncertainty, not a false clear, and it must not be
    # counted as cross-contract just because no same-slot pair matches either.
    df = pl.DataFrame({"date_": [1, 2], "settlement_c1": [100.0, 110.0],
                       "settlement_c2": [200.0, 220.0],
                       "ret1_adjusted": [None, 0.5]})
    assert cross_contract_cells(df).height == 0


def test_a_null_shipped_return_is_never_flagged():
    df = pl.DataFrame({"date_": [1, 2], "settlement_c1": [100.0, 110.0],
                       "settlement_c2": [200.0, 999.0],
                       "ret1_adjusted": [None, None]})
    assert cross_contract_cells(df).height == 0


def test_enumerates_slots_beyond_c1_and_c2():
    # date_ 2's shipped value reproduces ONLY from settlement_c4(t)/settlement_c3(t-1)
    # -- cross-slot and outside {c1, c2} -- which proves c3/c4 are actually
    # enumerated by the detector, not just carried through unused.
    df = pl.DataFrame({
        "date_": [1, 2],
        "settlement_c1": [100.0, 1000.0],
        "settlement_c2": [200.0, 2000.0],
        "settlement_c3": [300.0, 3000.0],
        "settlement_c4": [10.0, 33.0],
        "ret1_adjusted": [None, 33.0 / 300.0 - 1],
    })
    assert cross_contract_cells(df)["date_"].to_list() == [2]


def test_a_row_two_distinct_same_slot_pairs_both_match_is_reported_ambiguous():
    # settlement_c2 tracks settlement_c1 at a constant 2x, so date_ 2's return is
    # reproduced identically by (1, 1) and (2, 2) -- two distinct pairs. Still
    # correctly clean (a same-slot pair wins regardless of how many match), but
    # summarize() must surface the multiplicity rather than hide it.
    df = pl.DataFrame({"date_": [1, 2], "settlement_c1": [100.0, 110.0],
                       "settlement_c2": [200.0, 220.0],
                       "ret1_adjusted": [None, 0.10]})
    assert summarize(df) == {"shipped": 1, "clean": 1, "cross_contract": 0, "undecidable": 0, "ambiguous": 1}


def test_a_cross_contract_row_with_two_candidate_source_pairs_is_also_ambiguous():
    # settlement_c3(t)/settlement_c1(t-1) and settlement_c4(t)/settlement_c2(t-1)
    # both reproduce date_ 2's shipped value -- two different, equally valid
    # stories for which contract the number came from. Correctly cross-contract
    # (no same-slot pair matches either), but which pair explains it is genuinely
    # unresolved and must be reported, not silently picked.
    df = pl.DataFrame({
        "date_": [1, 2],
        "settlement_c1": [100.0, 500.0],
        "settlement_c2": [200.0, 500.0],
        "settlement_c3": [50.0, 110.0],
        "settlement_c4": [50.0, 220.0],
        "ret1_adjusted": [None, 0.10],
    })
    counts = summarize(df)
    assert counts["cross_contract"] == 1
    assert counts["ambiguous"] == 1


# ---------------------------------------------------------------------------
# Mutation tests against a known oracle. attach_front_slot (the fix's own
# slot-selection logic) is used ONLY to build ground-truth fixtures here -- the
# detector itself never receives a front_slot column.
# ---------------------------------------------------------------------------


def _synthetic_panel(settlement_c1: list[float], settlement_c2: list[float], expiring_this_month: list[int]) -> pl.DataFrame:
    raw = pl.DataFrame({
        "date_": list(range(len(settlement_c1))),
        "expiring_this_month": expiring_this_month,
        "settlement_c1": settlement_c1,
        "settlement_c2": settlement_c2,
    })
    slotted = attach_front_slot(raw)
    return slotted.with_columns(
        (pl.col("front_month_settlement") / pl.col("front_month_settlement").shift(1) - 1).alias("ret1_adjusted")
    )


def test_mutation_a_panel_of_only_same_slot_returns_has_zero_false_positives():
    # No expiry transition anywhere and neither series ever goes null, so
    # front_slot never moves: every shipped return is genuinely same-slot.
    panel = _synthetic_panel(
        settlement_c1=[100.0, 101.0, 102.5, 101.0, 99.0, 103.0],
        settlement_c2=[110.0, 112.0, 111.5, 109.0, 108.0, 107.0],
        expiring_this_month=[0, 0, 0, 0, 0, 0],
    )
    assert cross_contract_cells(panel).height == 0


def test_mutation_a_deliberately_planted_cross_contract_cell_is_caught():
    panel = _synthetic_panel(
        settlement_c1=[100.0, 101.0, 102.5, 101.0, 99.0, 103.0],
        settlement_c2=[110.0, 112.0, 111.5, 109.0, 108.0, 107.0],
        expiring_this_month=[0, 0, 0, 0, 0, 0],
    )
    # Overwrite one shipped return with settlement_c2(3) / settlement_c1(2) - 1 --
    # a ratio between two different contracts on a row where neither end's slot
    # ever moved, i.e. a value the guard would never legitimately produce here.
    c2_at_3 = panel.filter(pl.col("date_") == 3)["settlement_c2"].item()
    c1_at_2 = panel.filter(pl.col("date_") == 2)["settlement_c1"].item()
    planted = c2_at_3 / c1_at_2 - 1
    mutated = panel.with_columns(
        pl.when(pl.col("date_") == 3).then(pl.lit(planted)).otherwise(pl.col("ret1_adjusted")).alias("ret1_adjusted")
    )
    assert cross_contract_cells(mutated)["date_"].to_list() == [3]


_REAL_DEBUG_TABLE = TICKHISTORY_PATH / "debug" / "tables" / "BRN.csv"


@pytest.mark.skipif(not _REAL_DEBUG_TABLE.exists(), reason="real tickhistory debug table absent")
def test_mutation_a_hand_injected_cell_in_a_real_debug_table_is_caught():
    real = pl.read_csv(_REAL_DEBUG_TABLE, try_parse_dates=True, infer_schema_length=None).sort("date_").select(
        ["date_", "settlement_c1", "settlement_c2", "settlement_c3", "settlement_c4", "ret1_adjusted"]
    )
    baseline_flagged = set(cross_contract_cells(real)["date_"].to_list())

    # Build a cross-contract candidate value (settlement_c2(t) / settlement_c1(t-1))
    # from the row's own real prices, then find a row already outside the
    # baseline-flagged set where that candidate is genuinely different from the
    # value that actually shipped -- so injecting it is a real mutation, not a
    # no-op that happens to already equal ret1_adjusted.
    augmented = real.with_columns(
        (pl.col("settlement_c2") / pl.col("settlement_c1").shift(1) - 1).alias("_candidate")
    )
    pool = augmented.filter(
        pl.col("_candidate").is_not_null()
        & pl.col("ret1_adjusted").is_not_null()
        & (~pl.col("date_").is_in(list(baseline_flagged)))
        & ((pl.col("_candidate") - pl.col("ret1_adjusted")).abs() > 1e-6)
    )
    assert pool.height > 0, "no row in BRN.csv was suitable for a hand-injected mutation"
    target_date = pool["date_"][0]
    injected_value = pool["_candidate"][0]

    mutated = real.with_columns(
        pl.when(pl.col("date_") == target_date).then(pl.lit(injected_value))
        .otherwise(pl.col("ret1_adjusted")).alias("ret1_adjusted")
    )
    new_flagged = set(cross_contract_cells(mutated)["date_"].to_list())

    # Exactly the injected cell is newly caught -- nothing else in the table
    # shifts, because each row's classification depends only on its own and its
    # predecessor's settlement prices, never on a neighbour's ret1_adjusted.
    assert new_flagged == baseline_flagged | {target_date}


def test_the_detector_imports_nothing_from_the_guard_or_rescue_modules():
    # Literal check on the import lines, mirroring how this was verified by hand:
    # scripts/detect_sync_cross_contract.py never imports
    # globalmacro.pipeline.tickhistory (home of provenance_break_mask,
    # apply_provenance_guard, attach_front_slot, rescue_ret1_adjusted) or
    # globalmacro.utils.trade_presence (home of is_rescuable_mask).
    source = (PROJECT_ROOT / "scripts" / "detect_sync_cross_contract.py").read_text()
...
# ---------------------------------------------------------------------------
# summarize() / run(): the aggregation the CLI reports from. Written directly to
# temp CSVs -- these are the shape of a real debug table's settlement grid, not a
# monolithic fixture -- so a regression in the file-scanning logic itself (as
# opposed to the per-row classification already covered above) has a test.
# ---------------------------------------------------------------------------


def _write_debug_table(root: Path, name: str, rows: dict) -> Path:
    path = root / f"{name}.csv"
    pl.DataFrame(rows).write_csv(path)
    return path


def test_run_aggregates_summarize_counts_across_every_table_in_a_directory(tmp_path):
    _write_debug_table(tmp_path, "CLEAN", {
        "date_": [1, 2], "settlement_c1": [100.0, 110.0], "settlement_c2": [200.0, 220.0],
        "ret1_adjusted": [None, 0.10],
    })
    _write_debug_table(tmp_path, "DEFECT", {
        "date_": [1, 2], "settlement_c1": [100.0, 110.0], "settlement_c2": [200.0, 220.0],
        "ret1_adjusted": [None, 220.0 / 100.0 - 1],
    })
    totals = run(tmp_path)
    assert totals["files"] == 2
    assert totals["shipped"] == 2
    assert totals["clean"] == 1
    assert totals["cross_contract"] == 1


def test_run_skips_files_without_a_settlement_grid(tmp_path):
    _write_debug_table(tmp_path, "NOT_A_DEBUG_TABLE", {"date_": [1, 2], "some_other_col": [1, 2]})
